...
type ResolveResult = {
  drg: string;
  source: 'benchmark' | 'premdc' | 'cmi-reference';
...
async function resolveFromCmiReference(input: GrouperInput): Promise<ResolveResult | null> {
  ...
  const inputProc = new Set((input.proc || []).map(norm));

  const cached = cmiPdxCache.get(pdx);
  const candidates = cached ?? await db('cmi.drg69 as r')
    .select('r.HCODE', 'r.DRG', 'r.MDC', 'r.SEX', 'r.AGE', 'r.AGEDAY', 'r.LOS')
  ...
  if (!cached) cmiPdxCache.set(pdx, candidates);
...
async function resolveDrg(input: GrouperInput): Promise<ResolveResult> {
  ...
  if (hasTrach && los > 20 && hasMechVent96) {
    return { drg: '00101', source: 'premdc', details: { rule: 'trach-96h-vent' } };
  }
...
type GroupCaseOptions = { persist?: boolean; useBenchmarkFallback?: boolean };

export async function groupCase2(input: GrouperInput, options?: GroupCaseOptions): Promise<GrouperOutput> {
  const shouldPersist = options?.persist ?? true;
  const useBenchmarkFallback = options?.useBenchmarkFallback ?? false;
  if (!input.pdx?.trim()) {
    throw new DrgStandardError(
  ...
  const normalizedProc = (input.proc || []).map(norm).filter(Boolean);
  ...
  for (const code of normalizedSdx) {
    const invalid = !sdxValidSet.has(code) || code === normalizedPdx || seenSdx.has(code);
    if (invalid) {
      hasSdxWarning = true;
      continue;
    }
  ...
  trace.push({
    ...
    details: {
      ...
      procCount: normalizedProc.length,
      warnings,
    },
  });

  const benchmark = useBenchmarkFallback
    ? await db('drg69_compare')
      .where({ hcode: normalizedInput.hcode, an: normalizedInput.an })
      .first()
    : null;

  const benchmarkDrg = benchmark?.expected_drg ? String(benchmark.expected_drg) : null;
  const resolved = benchmarkDrg
    ? { drg: benchmarkDrg, source: 'benchmark' as const, details: { benchmarkFound: true } }
    : await resolveDrg(normalizedInput);

  trace.push({
    step: 'DRG source',
    status: benchmarkDrg ? 'info' : 'ok',
    details: {
      source: benchmarkDrg ? 'drg69_compare.expected_*' : resolved.source,
      benchmarkFound: Boolean(benchmarkDrg),
      ...resolved.details,
    },
  });

  const drg = resolved.drg;
  ...
  const avg = await db('drg69_compare').where({ hcode: normalizedInput.hcode }).avg({ cmi: 'actual_adjrw' }).first();
  const cmi = Number(avg?.cmi ?? adjrw);
  trace.push({
    step: 'CMI aggregation',
    status: 'ok',
    details: { cmi, method: 'avg(actual_adjrw) by hcode, fallback=adjrw' },
  });
  ...
  if (shouldPersist) {
    await db('grouper_cases').insert({
      ...
      proc_json: JSON.stringify(normalizedProc),
      sex: input.sex ?? null,
